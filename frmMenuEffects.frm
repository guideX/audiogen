...
Begin VB.Form frmMenuEffects 
   ...
   End
End
Attribute VB_Name = "frmMenuEffects"
...
AlwaysOnTop Me, True
lMenus.mEffectsMenuVisible = True
...
lMenus.mEffectsMenuVisible = False
...
If LabelMouseMove(lblEffect(Index), Button) = True Then
    lMenus.mEffectsMenuIndex = Index
End If
...
Private Sub lblEffect_MouseUp(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)
If lSettings.sHandleErrors = True Then On Local Error Resume Next
If Err.Number <> 0 Then ErrorAid Err.Number, Err.Description, "Private Sub lblEffect_MouseUp(Index As Integer, Button As Integer, Shift As Integer, X As Single, Y As Single)"
End Sub

Private Sub lblPlay_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
If lSettings.sHandleErrors = True Then On Local Error Resume Next
If LabelMouseDown(lblPlay, Button) = True Then
    If LCase(lblPlay.Caption) = "play" Then
        frmMain.ctlEffects.Play
        lblPlay.Caption = "Stop"
    ElseIf LCase(lblPlay.Caption) = "stop" Then
        frmMain.ctlEffects.Stop
        lblPlay.Caption = "Play"
    End If
End If
If Err.Number <> 0 Then ErrorAid Err.Number, Err.Description, "Private Sub lblSaveAs_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)"
End Sub

Private Sub lblPlay_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)
If lSettings.sHandleErrors = True Then On Local Error Resume Next
If LabelMouseMove(lblPlay, Button) = True Then
    
End If
If Err.Number <> 0 Then ErrorAid Err.Number, Err.Description, "Private Sub lblSaveAs_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)"
End Sub

Private Sub lblSaveAs_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
If lSettings.sHandleErrors = True Then On Local Error Resume Next
If LabelMouseDown(lblSaveAs, Button) = True Then
    DoEvents
    msg = Trim(SaveDialog(Me, "Wave Files (*.wav)|*.*|", "Save wave as ...", CurDir))
    msg = left(msg, Len(msg) - 1)
    If Right(LCase(msg), 4) <> ".wav" Then
        msg = msg & ".wav"
    End If
    If lEffectsPresets.eStatus <> eClosed And lEffectsPresets.eStatus <> 0 Then
...
If lSettings.sHandleErrors = True Then On Local Error Resume Next
If LabelMouseMove(lblSaveAs, Button) = True Then
    lMenus.mEffectsMenuIndex = 12
End If
If Err.Number <> 0 Then ErrorAid Err.Number, Err.Description, "Private Sub lblSaveAs_MouseMove(Button As Integer, Shift As Integer, X As Single, Y As Single)"
End Sub
